...
var AdminUserBox = React.createClass({
  ...
  _onChange: function () {
    this.setState({data: AdminUserStore.getUsers()});
  },
...
var User = React.createClass({
  ...
  render: function () {
    ...
    return (
      <li className="user" >
        <a href={email}>{this.props.name}</a>
        <button disabled={deleteDisabled} onClick={this.handleDelete}>Delete</button>
      </li>
    );
  }
});
  ...
  render: function () {
    return (
      <form className="newUserAdminForm" onSubmit={this.handleSubmit}>
        <input type="text" placeholder="Your name" name="name" ref="name"/>
        <br />
        <input type="email" placeholder="hello@example.com" name="email" ref="email" />
        <br />
        <input type="password" name="password" ref="password" />
        <br />
